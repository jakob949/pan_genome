...
def main(
    ...
    if clust_method == "fuzzy":
        if mean is not None and sd is not None:
            print(f"Using user-supplied mean ({mean}) and sd ({sd}) for fuzzy clustering.")
        elif model_name in DEFAULT_PARAMS:
            mean, sd = DEFAULT_PARAMS[model_name]
            print(f"Using pre-calibrated mean ({mean}) and sd ({sd}) for model {model_name}.")
        else:
            mean, sd = 0.8, 0.05
            print(
        ...
        print(f"Final similarity thresholds: {low:.4f} → {up:.4f} ({N_THRESHOLDS} steps)")
    else:
