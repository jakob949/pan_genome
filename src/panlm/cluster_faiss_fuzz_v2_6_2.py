...
def fast_calculate_probabilities_optimized(columns, sim_thresholds, mean=None, sd=None):
    ...
    if use_weights:
        ...
        if total_weight_sum == 0: use_weights = False
    if not use_weights:
    ...
    for pid in protein_ids_list:
        ...
        best_cl, cl_score = max(c_dict.items(), key=lambda x: x[1]) if c_dict else ("", 0.0)
        best_cl_prob = cl_score / total_weight_sum
        
        row_cat_probs = cat_probs[idx]
        best_cat_idx = np.argmax(row_cat_probs)
        final_rows.append((best_cl, best_cl_prob, category_types[best_cat_idx], row_cat_probs[best_cat_idx]))

    columns["cluster_id"], columns["cluster_prob"], columns["category"], columns["category_prob"] = zip(*final_rows)
    return {"weight_info": {'weighting_enabled': use_weights, 'mean': mean, 'std_dev': sd}}, columns
...
def cluster_faiss_parallel(
    ...
    columns = {
        ...
        "strain": [str(pid).split('|')[1] if '|' in str(pid) else "" for pid in protein_ids_np]
    }
    ...
    if algorithm in ["hdbscan", "dbscan"]:  
        ...
        print(f"FAISS search & distance conversion: {time.time() - t_search_start:.2f}s")
        ...
        for label in unique_labels:
            strain_frac = len(cluster_strains[label]) / total_strains if total_strains > 0 else 0
        ...
        for i in tqdm(range(num_batches), desc="Fuzzy Clustering"):
            ...
            process_batch_numba(parents_2d, ranks_2d, sims, idxs, similarity_thresholds, start)
        
        all_labels = {}
        for i, st in enumerate(similarity_thresholds):
            all_labels[f"{st:.4f}"] = np.array([numba_find(parents_2d[i], j) for j in range(num_points)], dtype=np.int32)
        ...
        for st_val in similarity_thresholds:
            ...
            for i, l in enumerate(labels):
                s = all_strains_np[i]
                if s: cluster_strains[l].add(s)
            
            l_to_props = {}
            for l in unique_labels:
                s_frac = len(cluster_strains[l]) / total_strains if total_strains > 0 else 0
                cat = 'core' if s_frac >= core_threshold else ('shell' if s_frac >= shell_threshold else 'cloud')
                l_to_props[l] = (counts[np.where(unique_labels==l)[0][0]], cat)
                
            columns[f"ST_{st_key}_size"] = np.array([l_to_props[l][0] for l in labels])
            columns[f"ST_{st_key}_category"] = np.array([l_to_props[l][1] for l in labels])

        res_meta, columns = fast_calculate_probabilities_optimized(columns, similarity_thresholds, mean, sd)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--k", type=int, default=2000, help="k-NN for HDBSCAN, DBSCAN, or Fuzzy")
    ...
    if args.algorithm == "fuzzy":
        ...
        if args.mean is not None and args.sd is not None:
            ...
            min_st, max_st = max(0.0, args.mean - 3*args.sd), min(1.0, args.mean + 3*args.sd)
        ...
        process_batch_numba(dummy_p, dummy_r, np.random.rand(1, 2).astype('float32'), np.array([[0,1]], dtype=np.int64), np.array([0.5]), 0)
