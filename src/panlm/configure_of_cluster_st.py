...
import numpy as np
import torch
from panlm.embs import calculate_embeddings, concatenate_embeddings
...
_CALIB_DIR = (
    "fastas_for_calibration"  # Default subdirectory for downloaded calibration files
)
...
def _download_calibration_fastas(dest_dir: str = _CALIB_DIR) -> list[str]:
        ...
        with urllib.request.urlopen(_REPO_ZIP) as resp:
            with zipfile.ZipFile(io.BytesIO(resp.read())) as zf:
                ...
                if not downloaded_any:
                    print(
                        f"Warning: No files matching '*.fna' found within the '{prefix}' path in the repository ZIP."
                    )
    ...
    if not final_fasta_files:
        print(
            f"Warning: Download process completed, but no '*.fna' files were ultimately found in {dest_dir}."
        )
...
def ensure_calibration_embeddings(
    ...
    )  # Ensure dest_dir (output for embeddings) exists

    input_pattern_for_embeddings: str
    output_prefix_for_embeddings: (
        str  # This will be like os.path.join(dest_dir, "prefix")
    )
    ...
    print(
        f"Embeddings will be generated with prefix: {output_prefix_for_embeddings}_<basename>.pt"
    )
...
def ensure_embeddings(
    ...
    if not fasta_files:
        raise FileNotFoundError(
            f"No FASTA files found matching input pattern: {input_pattern}"
        )

    print(
        f"Found {len(fasta_files)} FASTA files to process from pattern: {input_pattern}"
    )
...
def concatenate_embeddings_cpu(
    ...
    for f in embedding_files:
        ...
        for pid, emb in data.items():
            ...
            if expected_dim is None:
                ...
                print(
                    f"Warning: skipping {pid} in {f} (dim {vec_len} != expected {expected_dim})"
                )
                continue
...
def find_valid_st_range_coverage_criteria(
    ...
    for pt_file in pt_files:
        ...
        try:
            ...
            if not emb_data:
                print(
                    f"Warning: No data in {pt_file}, skipping ST range calculation for this file."
                )
                file2P_dominant_embs_log[pt_file] = [0.0] * len(threshold_values)
                continue

            temp_embs = [
                e.cpu().numpy().reshape(1, -1)
                for e in emb_data.values()
                if e.numel() > 0 and e.ndim > 0
            ]  # ensure e is not scalar
            if not temp_embs:
                print(
                    f"Warning: No valid embeddings to process in {pt_file} after loading, skipping ST range calculation."
                )
                file2P_dominant_embs_log[pt_file] = [0.0] * len(threshold_values)
                continue
            ...
            for _s_val_idx, s_val in enumerate(threshold_values):
                if n_emb_file_mock == 0:
                    ...
                    if s_val < 0.75:
                        ...
                        num_clusters_mock = max(1, n_emb_file_mock // 5)
                    elif s_val < 0.95:
                        num_clusters_mock = max(1, n_emb_file_mock // 2)
                    else:
                        num_clusters_mock = n_emb_file_mock
        ...
        if n_emb_file == 0:  # Should have been caught by temp_embs check
            print(
                f"Warning: Zero embeddings in {pt_file} after processing, skipping index calculation."
            )
            file2P_dominant_embs_log[pt_file] = [0.0] * len(threshold_values)
            continue
        ...
        for _s_idx, s_val in enumerate(threshold_values):
            ...
            P_val = (
                (sum_embs_in_dominant_clusters / n_emb_file) if n_emb_file > 0 else 0.0
            )
            P_dominant_embs_values.append(P_val)

        file2P_dominant_embs_log[pt_file] = P_dominant_embs_values

        abs_diff_high_coverage = np.abs(
            np.array(P_dominant_embs_values) - target_P_high_coverage
        )
        idx_coarse_file = np.argmin(abs_diff_high_coverage)

        abs_diff_medium_coverage = np.abs(
            np.array(P_dominant_embs_values) - target_P_medium_coverage
        )
    ...
    if not all_idx_coarse_list or not all_idx_fine_list:
        print(
            "Warning: No valid ST indices found from any processed files. Cannot determine overall range."
        )
    ...
    if overall_min_idx > overall_max_idx:
        print(
            f"Warning: Overall min_idx ({overall_min_idx}) > overall_max_idx ({overall_max_idx}). "
...
def main_new():
    parser = argparse.ArgumentParser(
        description="Determine similarity threshold range using dominant cluster coverage."
    )
    parser.add_argument(
        "--dest_dir",
    ...
    parser.add_argument(
        ...
        default="onnx",
        help="Embedding acceleration mode",
    )
    ...
    )  # Adjusted from 512 to 32 as per example
    parser.add_argument(
        "--model_name",
    ...
    try:
        ...
        print(
            "Please check your --user_fasta_dir pattern or network connection for calibration file download."
        )
        return
    except Exception as e:
        print(
            f"An unexpected error occurred during embedding preparation/generation: {e}"
        )
        import traceback
    ...
    if not pt_files:
        print(
            "No embedding (.pt) files were found or generated. Cannot proceed. Exiting."
        )
        return
    ...
    print(
        f"  Target P for high coverage (coarse clustering): {args.target_P_high_coverage}"
    )
    print(
        f"  Target P for medium coverage (fine clustering): {args.target_P_medium_coverage}"
    )

    file2ST_indices, file2P_log, overall_min_idx, overall_max_idx = (
        find_valid_st_range_coverage_criteria(
            pt_files,
            args.alpha_dominant,
    ...
    if not file2ST_indices:
        ...
        for pt, (idx_coarse, idx_fine) in file2ST_indices.items():
            sim_coarse = (
                threshold_values[idx_coarse]
                if 0 <= idx_coarse < len(threshold_values)
                else float("nan")
            )
            sim_fine = (
                threshold_values[idx_fine]
                if 0 <= idx_fine < len(threshold_values)
                else float("nan")
            )
            print(
                f"  {os.path.basename(pt)}: coarse_idx={idx_coarse} (sim={sim_coarse:.4f}), "
                f"fine_idx={idx_fine} (sim={sim_fine:.4f})"
            )
            if not np.isnan(sim_coarse):
                mean_threshold_coarse_vals.append(sim_coarse)
            if not np.isnan(sim_fine):
                mean_threshold_fine_vals.append(sim_fine)

        if mean_threshold_coarse_vals:
            mean_coarse_sim = np.mean(mean_threshold_coarse_vals)
            print(
                f"  Mean of per-file coarse similarity thresholds: {mean_coarse_sim:.4f}"
            )
    ...
    lower_sim_overall = (
        ...
        if 0 <= overall_min_idx < len(threshold_values)
        else float("nan")
    )
    higher_sim_overall = (
        threshold_values[overall_max_idx]
        if 0 <= overall_max_idx < len(threshold_values)
        else float("nan")
    )

    print(
        f"  Lower similarity threshold bound: {lower_sim_overall:.4f} (index {overall_min_idx})"
    )
    print(
        f"  Higher similarity threshold bound: {higher_sim_overall:.4f} (index {overall_max_idx})"
    )
