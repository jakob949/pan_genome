...
@numba.njit
def process_batch_numba(parents_2d, ranks_2d, similarities, indices, similarity_thresholds, start_idx):
    """
    ...
    for i in range(num_points_in_batch):
        ...
        for k in range(1, similarities.shape[1]): # Start from 1 to skip self
            ...
            num_thresholds_passed = np.searchsorted(similarity_thresholds, sim, side='right')
...
def fast_calculate_probabilities_optimized(columns, sim_thresholds, mean=None, sd=None):
    ...
    unique_protein_ids, protein_indices = np.unique(protein_ids_list, return_inverse=True)
    ...
    weighted_category_scores = np.zeros((n_proteins, len(category_types)), dtype=np.float64)
    for st in sim_thresholds:
    ...
    category_probabilities_d = {pid: {cat: category_probs[idx, i] for i, cat in enumerate(category_types)} for pid, idx in protein_to_idx.items()}
    ...
    for pid, idx in protein_to_idx.items():
        ...
        if total_cluster_score > 0:
            cluster_probabilities[pid] = {k: v / total_cluster_score for k, v in weighted_cluster_scores[idx].items()}
        else:
    ...
    for pid in protein_ids_list:
        ...
        best_cluster, best_cluster_prob = max(c_probs.items(), key=lambda item: item[1]) if c_probs else ("", 0.0)
    ...
    columns["cluster_id"], columns["cluster_prob"], columns["category"], columns["category_prob"] = zip(*final_rows)

    print(f"--- Fast probability calculation took: {time.time() - t1:.2f} seconds ---")
    results = {"weight_info": {'weighting_enabled': use_weights, 'mean': mean, 'std_dev': sd}}
...
def cluster_faiss_parallel(
    ...
    columns = {"protein_id": protein_ids_np.tolist(), "strain": [str(pid).split('|')[1] if '|' in str(pid) else "" for pid in protein_ids_np]}
    ...
    if algorithm in ["hdbscan", "dbscan"]:
        if not HAS_HDBSCAN:
            raise ImportError(f"Sklearn clustering with HDBSCAN support is required for algorithm '{algorithm}'.")
        ...
        print(f"OPTIMIZATION: Setting index.nprobe={index.nprobe} to ensure valid neighbor retrieval.")
        ...
        for batch_idx in tqdm(range(num_batches_calc), desc="Fetching Neighbors"):
            ...
            all_data[current_idx : current_idx+valid_count] = flat_distances[valid_mask]
        ...
        knn_graph = csr_matrix((all_data, all_cols, indptr), shape=(num_points, num_points))
        
        del all_cols, all_data, indptr
        gc.collect()

        # --- Automatic Symmetry Detection Logic ---
        graph_size_bytes = knn_graph.data.nbytes + knn_graph.indices.nbytes + knn_graph.indptr.nbytes
        ...
        if should_symmetrize:
            ...
            try:
                ...
                print(f"WARNING: Symmetrization failed: {e}. Proceeding with directed graph.")
                gc.collect()
        ...
        if n_components > 1:
            print(f"Graph has {n_components} connected components. Patching to ensure connectivity...")
        ...
        print(f"Fitting Sklearn {algorithm.upper()} (via HDBSCAN engine) (Input: {knn_graph.shape} Sparse Matrix)...")
        ...
        if algorithm == 'dbscan':
            ...
            c_epsilon = eps
            c_method = 'leaf' # 'leaf' usually closer to DBSCAN flat extraction than 'eom'
            print(f"Configuring as DBSCAN: epsilon={c_epsilon}, min_samples={eff_min_samples}")
        else:
        ...
        label_to_props = {
            label: {
                'size': data['size'], 
                'category': 'core' if (len(data['strains']) / total_strains if total_strains > 0 else 0) >= core_threshold else ('shell' if (len(data['strains']) / total_strains if total_strains > 0 else 0) >= shell_threshold else 'cloud')
            } 
            for label, data in cluster_data.items()
        }

        prop_array_cat = np.array([label_to_props[l]['category'] for l in unique_labels])
        ...
        parents_2d = np.arange(num_points, dtype=np.int64)[None, :] * np.ones((num_thresholds, 1), dtype=np.int64)
        ...
        for batch_idx in tqdm(range(num_batches_calc), desc="Clustering"):
            ...
            process_batch_numba(parents_2d, ranks_2d, similarities, indices, similarity_thresholds, start)
        
        t_numba_proc_time = time.time() - t_search_start - total_faiss_search_time
        print(f"Time for search & processing loop: {time.time() - t_search_start:.2f} seconds ---")
        ...
        for i, st_val in enumerate(similarity_thresholds):
            final_labels = np.array([numba_find(parents_2d[i], j) for j in range(num_points)], dtype=np.int32)
            all_cluster_labels[f"{st_val:.4f}"] = final_labels
        print(f"Time for cluster label finalization: {time.time() - t_finalize_start:.2f} seconds ---")
        ...
        for st_val in similarity_thresholds:
            ...
            label_to_props = {label: {'size': data['size'], 'category': 'core' if (len(data['strains']) / total_strains if total_strains > 0 else 0) >= core_threshold else ('shell' if (len(data['strains']) / total_strains if total_strains > 0 else 0) >= shell_threshold else 'cloud')} for label, data in cluster_data.items()}
            
            prop_array_size = np.array([label_to_props[l]['size'] for l in unique_labels])
            prop_array_cat = np.array([label_to_props[l]['category'] for l in unique_labels])
            
            columns[f"ST_{st_key}_size"] = prop_array_size[labels_inverse]
            columns[f"ST_{st_key}_category"] = prop_array_cat[labels_inverse]

        print(f"Time for post-clustering analysis: {time.time() - t_post_start:.2f} seconds ---")
        
        results, columns = fast_calculate_probabilities_optimized(columns, similarity_thresholds, mean, sd)
        return columns, results

if __name__ == "__main__":
    t_script_start = time.time()
    parser = argparse.ArgumentParser(description="High-performance FAISS-based clustering with Numba.", formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    parser.add_argument("--input_file", required=True, help="Path to input embeddings (.npz or .pt), can use wildcards like '*.npz'")
    parser.add_argument("--output_file", required=True, help="Path to save the final CSV result file")
    
    parser.add_argument("--algorithm", type=str, default="fuzzy", choices=["fuzzy", "hdbscan", "dbscan"], help="Clustering algorithm to use. 'dbscan' mode uses HDBSCAN with flat selection.")
    parser.add_argument("--min_cluster_size", type=int, default=10, help="Minimum cluster size (HDBSCAN) or min_samples (DBSCAN).")
    parser.add_argument("--eps", type=float, default=0.1, help="Epsilon parameter for DBSCAN (distance threshold). Default 0.1")
        
    parser.add_argument("--core_threshold", type=float, default=0.95, help="Minimum strain fraction for a cluster to be 'core'")
    parser.add_argument("--shell_threshold", type=float, default=0.15, help="Minimum strain fraction for a cluster to be 'shell'")
    parser.add_argument("--cpu", action="store_true", help="Use CPU for FAISS indexing instead of GPU")
    parser.add_argument("--k", type=int, default=500, help="Number of nearest neighbors to consider for clustering")
    parser.add_argument("--batch_size", type=int, default=10000, help="Batch size for FAISS search")
    parser.add_argument("--st", type=str, default="0.825,9.5,25", help="Similarity thresholds as a string: 'low,high,steps' (Fuzzy only)")
    parser.add_argument("--mean", type=float, default=0.86, help="Mean for normal distribution weights on thresholds. If unset, weighting is disabled. (Fuzzy only)")
    parser.add_argument("--sd", type=float, default=0.01, help="Standard deviation for normal distribution weights. If unset, weighting is disabled. (Fuzzy only)")
    parser.add_argument("--pca_dim", type=int, default=None, help="Target dimension for PCA reduction. If not set, PCA is disabled.")
    ...
    if args.algorithm == "fuzzy":
        ...
        if args.mean is not None and args.sd is not None:
            print("defining similarity thresholds based on normal distribution parameters")
            min_st = max(0.0, args.mean - 3 * args.sd)
            max_st = min(1.0, args.mean + 3 * args.sd)
            similarity_thresholds = np.linspace(min_st, max_st, 15)
            print(f"Using similarity thresholds from {min_st:.4f} to {max_st:.4f} based on normal distribution")
            print(f"similarity thresholds: {similarity_thresholds}")
        else:
            similarity_thresholds = np.linspace(low, high, int(steps)) if int(steps) > 1 else np.array([low])
        
        print("Pre-compiling Numba functions...")
        try:
            dummy_parents = np.arange(2, dtype=np.int64)[None,:]
            dummy_ranks = np.zeros((1, 2), dtype=np.int64)
            process_batch_numba(dummy_parents, dummy_ranks, np.random.rand(1, 2).astype('float32'), np.array([[0,1]], dtype=np.int64), np.array([0.5]), 0)
            print("Compilation complete.")
        except Exception as e:
            print(f"Could not pre-compile Numba functions. This may happen on some systems. Continuing without pre-compilation. Error: {e}")
    else:
        similarity_thresholds = None

    columns, results = cluster_faiss_parallel(
        args.input_file, similarity_thresholds, core_threshold=args.core_threshold,
        shell_threshold=args.shell_threshold, cpu=args.cpu, k=args.k,
        batch_size=args.batch_size, mean=args.mean, sd=args.sd, pca_dim=args.pca_dim,
        algorithm=args.algorithm, min_cluster_size=args.min_cluster_size, eps=args.eps
    )
